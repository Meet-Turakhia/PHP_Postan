...
<body>
    ...
    <div class="container mt-5">
        ...
        <div class="row">
            ...
            <div class="col-sm">
                <h2><kbd style="color: #ffcc00; background-color:black; font-family: Century Gothic; border-radius: 10px;">Dash</kbd>Board</h2>
            </div>

            <div class="col-sm">
                <?php
                if ($u_type == "Customer") {
                    $count_query = $mysqli->query("SELECT * FROM courier WHERE customer_id = '$u_id' ORDER BY reg_date DESC");
                } elseif ($u_type == "Delivery Boy") {
                    $count_query = $mysqli->query("SELECT * FROM courier WHERE delboy_id = '$u_id' ORDER BY reg_date DESC");
                } else {
                    $count_query = $mysqli->query("SELECT * FROM courier ORDER BY reg_date DESC");
                }
                $count = 0;
                ...
                ?>
                <h2 id="main_title">Active <span id="primarycolor">Orders:</span> <?php echo $count ?> </h2>
            </div>
        ...
        <div class="row row-cols-1 row-cols-md-2" style="margin-top: 20px;">
            ...
            <?php
            if ($u_type == "Customer") {
                $result = $mysqli->query("SELECT *, users.address as user_address, courier.address as courier_address FROM courier INNER JOIN users ON courier.customer_id = users.id WHERE customer_id = '$u_id' ORDER BY upd_date DESC");
                $result2 = $mysqli->query("SELECT *, users.address as user_address, courier.address as courier_address FROM courier INNER JOIN users ON courier.delboy_id = users.id WHERE customer_id = '$u_id' ORDER BY upd_date DESC");
            } elseif ($u_type == "Delivery Boy") {
                $result = $mysqli->query("SELECT *, users.address as user_address, courier.address as courier_address FROM courier INNER JOIN users ON courier.customer_id = users.id WHERE delboy_id = '$u_id' ORDER BY upd_date DESC");
                $result2 = $mysqli->query("SELECT *, users.address as user_address, courier.address as courier_address FROM courier INNER JOIN users ON courier.delboy_id = users.id WHERE delboy_id = '$u_id' ORDER BY upd_date DESC");
            } else {
                $result = $mysqli->query("SELECT *, users.address as user_address, courier.address as courier_address FROM courier INNER JOIN users ON courier.customer_id = users.id ORDER BY upd_date DESC");
                $result2 = $mysqli->query("SELECT uname FROM (SELECT uname, id FROM users) AS u INNER JOIN (SELECT delboy_id, upd_date FROM courier) AS c ON c.delboy_id = u.id ORDER BY c.upd_date DESC");
            }
            ...
            ?>
                <div class="col mb-4">
                    <div class="card" style="border-radius: 25px;">
                        <a href="viewcourier.php?view_id=<?php echo $row["courier_id"] ?>" style="overflow: hidden; position: absolute; top: 10px;right: 10px;"><i class='fas fa-chevron-circle-right' id="ch" title="View Details"></i></a>
                        <?php echo "<img src='data:image/jpeg;base64," . base64_encode($row["image"]) . "' alt='Courier image' style='max-width: 150px; margin-top: 10px; margin-left: 10px; border-radius: 10px;'>"; ?>
                        <div class="card-body">
                            <center>
                                <h5 class="card-title font-weight-bold"> <?php echo $row["title"]; ?> </h5>
                            </center>
                            <h7>Registered: <b> <?php echo $row["reg_date"]; ?> </b></h7>
                            <h7 style="float: right; overflow: hidden;" id="status">Status:<b> <?php echo $row["status"]; ?> </b></h7>
                            <?php if ($row["reg_date"] != $row["upd_date"]) { ?>
                                <br>
                                <h7>Updated: <b> <?php echo $row["upd_date"]; ?> </b></h7>
                            <?php } ?>
                            <div class="card-footer text-muted ca" style="max-width: 100%;">
                                ...
                                <a>Source: <b><?php echo $row["user_address"]; ?></b> <br>
                                    Destination: <b> <?php echo $row["courier_address"]; ?> </b>
                                </a>
                            </div>
                        </div>
                    </div>
